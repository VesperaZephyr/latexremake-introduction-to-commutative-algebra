...
The ring $S^{-1}A$ is called the \emph{ring of fractions}\index{ring of fractions}\index{localization} of $A$ with respect to $S$. It has a universal property:

\begin{proposition}\label{prop:universal-property}\index{universal property!of localization}
...
\begin{examples}\label{ex:localization-examples}\index{localization!examples}
\begin{enumerate}[(1)]
...
\begin{proposition}\label{prop:localization-exact}\index{exact functor!localization}
...
\begin{corollary}\label{cor:flat-localization}\index{flat module!localization}
...
A property $P$ of a ring $A$ (or of an $A$-module $M$) is said to be a \emph{local property}\index{local property} if the following is true:
\[
...
\begin{proof}
Clearly (i) $\implies$ (ii) $\implies$ (iii). Suppose (iii) satisfied and $M \neq 0$. Let $x$ be a non-zero element of $M$, and let $\mathfrak{a} = \Ann(x)$; $\mathfrak{a}$ is an ideal $\neq (1)$, hence is contained in a maximal ideal $\mathfrak{m}$ by Proposition~1.4. Consider $x/1 \in M_{\mathfrak{m}}$. Since $M_{\mathfrak{m}} = 0$ we have $x/1 = 0$, hence $x$ is killed by some element of $A \setminus \mathfrak{m}$; but this is impossible since $\Ann(x) \subseteq \mathfrak{m}$.
\end{proof}
...
\begin{proposition}\label{prop:flat-local}\index{flat module!local property}
