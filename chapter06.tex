...
i) is called the \emph{ascending chain condition} (a.c.c. for short) and ii) the \emph{maximal condition}. A module $M$ satisfying either of these equivalent conditions is said to be \emph{Noetherian}\index{Noetherian, module} (after Emmy Noether). If $\Sigma$ is ordered by $\supseteq$, then i) is the \emph{descending chain condition} (d.c.c. for short) and ii) the \emph{minimal condition}. A module $M$ satisfying these is said to be \emph{Artinian}\index{Artinian, module} (after Emil Artin).
...
A ring $A$ is said to be \emph{Noetherian}\index{Noetherian, module!ring} (resp.\ \emph{Artinian})\index{Artinian, module!ring} if it is so as an $A$-module, i.e., if it satisfies a.c.c.\ (resp.\ d.c.c.) on ideals.
...
A \emph{chain} of submodules \index{Chain of submodules} of a module $M$ is a sequence $(M_i)$ ($0 \leq i \leq n$) of submodules of $M$ such that $M = M_0 \supset M_1 \supset \cdots \supset M_n = 0$ (strict inclusions). The \emph{length} of the chain is $n$ (the number of ``links''). A \emph{composition series}\index{Composition series} of $M$ is a maximal chain, that is one in which no extra submodules can be inserted: this is equivalent to saying that each quotient $M_{i-1}/M_i$ ($1 \leq i \leq n$) is \emph{simple} (that is, has no submodules except $0$ and itself).
...
A module satisfying both a.c.c.\ and d.c.c.\ is therefore called a \emph{module of finite length}. By \eqref{prop:composition-series} all composition series of $M$ have the same length $l(M)$, called the \emph{length} of $M$. The \emph{Jordan--H\"older theorem} applies to modules of finite length: if $(M_i)_{0 \leq i \leq n}$ and $(M'_i)_{0 \leq i \leq m}$ are any two composition series of $M$, there is a one-to-one correspondence between the set of quotients $(M_{i-1}/M_i)_{1 \leq i \leq n}$ and the set of quotients $(M'_{i-1}/M'_i)_{1 \leq i \leq m}$ such that corresponding quotients are isomorphic. The proof is the same as for finite groups.
