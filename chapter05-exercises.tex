...
\begin{enumerate}
    ...
    \emph{Noether's normalization lemma.}\index{Normalization lemma}
    ...
    \emph{Nullstellensatz (weak form).}\index{Hilbert Nullstellensatz}
    ...
    A ring $A$ with the three equivalent properties above is called a \emph{Jacobson ring}.
    ...
    \item \label{exer26-chapter5}Let $X$ be a topological space. A subset of $X$ is \emph{locally closed} if it is the intersection of an open set and a closed set, or equivalently if it is open in its closure.
    ...
    \end{enumerate}
    \quad\, A subset $X_0$ satisfying these conditions is said to be \emph{very dense} in $X$.
