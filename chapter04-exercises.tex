...
\begin{enumerate}
    ...
    \item \label{exer12-chapter4}Let $A$ be a ring, $S$ a multiplicatively closed subset of $A$. For any ideal $\mathfrak{a}$, let $S(\mathfrak{a})$ denote the contraction of $S^{-1}\mathfrak{a}$ in $A$. The ideal $S(\mathfrak{a})$ is called the \emph{saturation} of $\mathfrak{a}$ with respect to $S$. Prove that:
    \begin{enumerate}[i)]
...
\begin{enumerate}
    \setcounter{enumi}{19}
    \item \label{exer20-chapter4}Let $M$ be a fixed $A$-module, $N$ a submodule of $M$. The \emph{radical of $N$ in $M$}\index{Radical, Jacobson!of submodule} is defined to be
    \[
    ...
    \item \label{exer21-chapter4}An element $x \in A$ defines an endomorphism $\phi_x$ of $M$, namely $m \mapsto xm$. The element $x$ is said to be a \emph{zero-divisor} (resp. \emph{nilpotent}) in $M$ if $\phi_x$ is not injective (resp. is nilpotent). A submodule $Q$ of $M$ is \emph{primary in $M$} if $Q \neq M$ and every zero-divisor in $M/Q$ is nilpotent.
