...
Let $M, N$ be $A$-modules. A mapping $f \colon M \to N$ is an \emph{$A$-module homomorphism}\index{Module(s)!homomorphism} (or is $A$-linear) if
\begin{align*}
    f(x + y) &= f(x) + f(y), \\
    f(ax) &= af(x)
\end{align*}
...
%=============================================================================
\subsec{Submodules and Quotient Modules}

A \emph{submodule}\index{Module(s)!sub} $M'$ of $M$ is a subgroup of $M$ which is closed under multiplication by elements of $A$. The abelian group $M/M'$ then inherits an $A$-module structure from $M$, defined by $a(x + M') = ax + M'$. The $A$-module $M/M'$ is the \emph{quotient}\index{Module(s)!quotient} of $M$ by $M'$.

If $f \colon M \to N$ is an $A$-module homomorphism, the \emph{kernel}of $f$ is
\[
    \Ker(f) = \{x \in M : f(x) = 0\}
\]
and is a submodule of $M$. The \emph{image} of $f$ is
\[
    \Image(f) = f(M)
\]
...
\subsec{Operations on Submodules}

Most of the operations on ideals considered in Chapter 1 have their counterparts for modules. Let $M$ be an $A$-module and let $(M_i)_{i \in I}$ be a family of submodules of $M$. Their \emph{sum}\index{Module(s)!sum of} $\sum M_i$ is the set of all (finite) sums $\sum x_i$, where $x_i \in M_i$ for all $i \in I$, and almost all the $x_i$ (that is, all but a finite number) are zero. $\sum M_i$ is the smallest submodule of $M$ which contains all the $M_i$.
...
We cannot in general define the \emph{product}\index{Module(s)!product of} of two submodules, but we can define the product $\mathfrak{a}M$, where $\mathfrak{a}$ is an ideal and $M$ an $A$-module; it is the set of all finite sums $\sum a_i x_i$ with $a_i \in \mathfrak{a}$, $x_i \in M$, and is a submodule of $M$.
...
An $A$-module is \emph{faithful}\index{Module(s)!faithful} if $\Ann(M) = 0$. If $\Ann(M) = \mathfrak{a}$, then $M$ is faithful as an $A/\mathfrak{a}$-module.
...
If $x$ is an element of $M$, the set of all multiples $ax$ $(a \in A)$ is a submodule of $M$, denoted by $Ax$ or $(x)$. If $M = \sum_{i \in I} Ax_i$, the $x_i$ are said to be a \emph{set of generators}\index{Generators of a module} of $M$; this means that every element of $M$ can be expressed (not necessarily uniquely) as a finite linear combination of the $x_i$ with coefficients in $A$. An $A$-module $M$ is said to be \emph{finitely generated}\index{Module(s)!finitely-generated} if it has a finite set of generators.
%=============================================================================
\subsec{Direct Sum and Product}

If $M, N$ are $A$-modules, their \emph{direct sum}\index{Module(s)!direct sum of} $M \oplus N$ is the set of all pairs $(x, y)$ with $x \in M$, $y \in N$, with componentwise addition and scalar multiplication in the obvious way:
\begin{align*}
    (x_1, y_1) + (x_2, y_2) &= (x_1 + x_2, y_1 + y_2), \\
    a(x, y) &= (ax, ay).
\end{align*}

More generally, if $(M_i)_{i \in I}$ is any family of $A$-modules, we can define their \emph{direct sum} $\bigoplus_{i \in I} M_i$; its elements are families $(x_i)_{i \in I}$ such that $x_i \in M_i$ for each $i \in I$ and almost all $x_i$ are $0$. If we drop the restriction on the number of non-zero $x_i$'s we have the \emph{direct product}\index{Module(s)!direct product of} $\prod_{i \in I} M_i$. Direct sum and direct product are therefore the same if the index set $I$ is finite, but not otherwise, in general.
...
\]
with $a_i \in A_i$ is an \emph{ideal} $\mathfrak{a}_i$ of $A$ (it is not a subring of A---except in trivial cases---because it does not contain the identity element of $A$). The ring $A$, considered as an $A$-module, is the direct sum of the ideals $\mathfrak{a}_1, \ldots, \mathfrak{a}_n$. Conversely, given a module decomposition
\[
...
%=============================================================================
\subsec{Finitely Generated Modules}

A \emph{free}\index{Module(s)!free} $A$-module is one which is isomorphic to an $A$-module of the form $\bigoplus_{i \in I} M_i$, where each $M_i \cong A$ (as an $A$-module). A finitely generated free $A$-module is therefore isomorphic to $A \oplus \cdots \oplus A$ ($n$ summands), which is denoted by $A^n$.
...
\begin{proposition}[Nakayama's Lemma]\label{pro:nakayama}\index{Nakayama's Lemma}
...
\begin{proof}[First Proof]
    By \eqref{cor:cayley-hamilton} we have $xM = 0$ for some $x \equiv 1 \pmod{\mathfrak{a}}$. By \eqref{prop:jacobson-radical}, $x$ is a unit in $A$, hence $M = x^{-1}xM = 0$.
\end{proof}
...
A sequence of type (3) is called a \emph{short exact sequence} Any long exact sequence (0) can be split up into short exact sequences: if $N_i = \Image(f_i) = \Ker(f_{i+1})$, we have short exact sequences $0 \longrightarrow N_i \longrightarrow M_i \longrightarrow N_{i+1} \longrightarrow 0$ for each $i$.
...
Let $\mathcal{C}$ be a class of $A$-modules and let $\lambda$ be a function on $\mathcal{C}$ with values in $\Z$ (or, more generally, with values in an abelian group $G$). The function $\lambda$ is \emph{additive}\index{Additive function} if, for each short exact sequence
\[
...
%=============================================================================
\subsec{Tensor Product of Modules}
...
We shall construct an $A$-module $T$, called the \textit{tensor product}\index{Tensor product, of algebras!of modules} of $M$ and $N$, with the property that the $A$-bilinear mappings $M \times N \to P$ are in a natural one-to-one correspondence with the $A$-linear mappings $T \to P$, for all $A$-modules $P$. More precisely:
...
\subsec{Restriction and extension of scalars}\index{Scalars, extension of}\index{Scalars, extension of!restriction of}

Let $f \colon A \to B$ be a homomorphism of rings and let $N$ be a $B$-module. Then $N$ has an $A$-module structure defined as follows: if $a \in A$ and $x \in N$, then $ax$ is defined to be $f(a)x$. This $A$-module is said to be obtained from $N$ by \emph{restriction of scalars}. In particular, $f$ defines in this way an $A$-module structure on $B$.
...
%=============================================================================
\subsec{Exactness Properties of the Tensor Product}
...
The functor $T_N \colon M \mapsto M \otimes_A N$ on the category of $A$-modules and homomorphisms is therefore not in general exact. If $T_N$ is exact, that is to say if tensoring with $N$ transforms all exact sequences into exact sequences, then $N$ is said to be a \emph{flat}\index{Flat}\index{Module(s)!flat} $A$-module.
...
\begin{remark}
\begin{enumerate}[i)]
    \item In particular, if $A$ is a field $K$ (and $B \neq 0$) then $f$ is injective by \eqref{prop:field-characterization} and therefore $K$ can be canonically identified with its image in $B$. Thus a $K$-algebra ($K$ a field) is effectively a ring containing $K$ as a subring.
...
Let $f \colon A \to B$, $g \colon A \to C$ be two ring homomorphisms. An \emph{$A$-algebra homomorphism}\index{Algebra!homomorphism} $h \colon B \to C$ is a ring homomorphism which is also an $A$-module homomorphism. The reader should verify that $h$ is an $A$-algebra homomorphism if and only if $h \circ f = g$.

A ring homomorphism $f \colon A \to B$ is \emph{finite}, and $B$ is a \emph{finite $A$-algebra}, if $B$ is finitely generated as an $A$-module. The homomorphism $f$ is of \emph{finite type}\index{Finite, A-Algebra!type}, and $B$ is a \emph{finitely-generated $A$-algebra}\index{Finitely-generated}\index{Finite, A-Algebra}, if there exists a finite set of elements $x_1, \ldots, x_n$ in $B$ such that every element of $B$ can be written as a polynomial in $x_1, \ldots, x_n$ with coefficients in $f(A)$; or equivalently if there is an $A$-algebra homomorphism from a polynomial ring $A[t_1, \ldots, t_n]$ onto $B$.

A ring $A$ is said to be \emph{finitely generated} if it is finitely generated as a $\Z$-algebra. This means that there exist finitely many elements $x_1, \ldots, x_n$ in $A$ such that every element of $A$ can be written as a polynomial in the $x_i$ with rational integer coefficients.

%=============================================================================
\subsec{Tensor Product of Algebras}\index{Tensor product, of algebras}
