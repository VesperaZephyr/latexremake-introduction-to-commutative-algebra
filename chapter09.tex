...
\subsec{Discrete Valuation Rings}
...
An integral domain $A$ is a \emph{discrete valuation ring}\index{Discrete valuation ring}\index{Ring!discrete valuation} if there is a discrete valuation $v$ of its field of fractions $K$ such that $A$ is the valuation ring of $v$. By \eqref{prop:5.18}, $A$ is a local ring, and its maximal ideal $\mathfrak{m}$ is the set of all $x \in K$ such that $v(x) > 0$.
...
\subsec{Fractional Ideals}
Let $A$ be an integral domain, $K$ its field of fractions. An $A$-submodule $M$ of $K$ is a \emph{fractional ideal}\index{Ideal(s)!fractional} of $A$ if $xM \subseteq A$ for some $x \neq 0$ in $A$. In particular, the ``ordinary'' ideals (now called integral ideals) are fractional ideals (take $x = 1$).

Any element $u \in K$ generates a fractional ideal, denoted by $(u)$ or $Au$, and called \emph{principal}. If $M$ is a fractional ideal, the set of all $x \in K$ such that $xM \subseteq A$ is denoted by $(A : M)$.
...
This group is called the \emph{group of ideals} of $A$; we denote it by $I$. In this terminology \eqref{cor:dedekind-unique-factorization} says that $I$ is a free (abelian) group, generated by the non-zero prime ideals of $A$.

Let $K^*$ denote the multiplicative group of the field of fractions $K$ of $A$. Each $u \in K^*$ defines a fractional ideal $(u)$, and the mapping $u \mapsto (u)$ is a homomorphism $\varphi \colon K^* \to I$. The image $P$ of $\varphi$ is the group of \emph{principal fractional ideals}: the quotient group $H = I/P$ is called the \emph{ideal class group} of $A$. The kernel $U$ of $\varphi$ is the set of all $u \in K^*$ such that $(u) = (1)$, so that it is the group of units of $A$. We have an exact sequence:
\[
...
\begin{examples}
\begin{enumerate}[1)]
    \item $K = \Q(\sqrt{-1})$; $n = 2$, $r_1 = 0$, $r_2 = 1$, $r_1 + r_2 - 1 = 0$. The only units in $\Z[\mathrm{i}] = A$ are the four roots of unity $\pm 1$, $\pm i$.
