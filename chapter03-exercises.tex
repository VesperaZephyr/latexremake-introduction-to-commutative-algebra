...
\begin{enumerate}
    ...
    \item \label{exer7-chapter3}A multiplicatively closed subset $S$ of a ring $A$ is said to be \emph{saturated} if $xy \in S \iff x \in S \text{ and } y \in S$. Prove that:
    \begin{enumerate}[i)]
    ...
    $B$ is said to be \emph{faithfully flat}\index{faithfully flat} over $A$.
    ...
    \end{enumerate}
    The assignment of the ring $A(U)$ to each basic open set $U$ of $X$, and the restriction homomorphisms $\rho$, satisfying the conditions iii) and iv) above, constitutes a \emph{presheaf of rings}\index{presheaf} on the basis of open sets $(X_f)_{f \in A}$. v) says that the \emph{stalk}\index{stalk} of this presheaf at $x \in X$ is the corresponding local ring $A_{\mathfrak{p}}$.
