...
A prime ideal in a ring $A$ is in some sense a generalization of a prime number. The corresponding generalization of a power of a prime number is a primary ideal\index{Primary, decomposition!ideal}. An ideal $\mathfrak{q}$ in a ring $A$ is primary if $\mathfrak{q} \neq A$ and if
\[
...
If $\mathfrak{p} = r(\mathfrak{q})$, then $\mathfrak{q}$ is said to be \emph{$\mathfrak{p}$-primary}.
...
\begin{lemma}\label{lem:colon-primary}
...
The prime ideals $\mathfrak{p}_i$ in \eqref{thm:first-uniqueness} are said to \emph{belong} to $\mathfrak{a}$, or to be \emph{associated}\index{Prime ideal(s)!associated} with $\mathfrak{a}$. The ideal $\mathfrak{a}$ is primary if and only if it has only one associated prime ideal. The minimal elements of the set $\{\mathfrak{p}_1, \ldots, \mathfrak{p}_n\}$ are called the \emph{minimal} or \emph{isolated}\index{Prime ideal(s)!isolated} prime ideals belonging to $\mathfrak{a}$. The others are called \emph{embedded}\index{Prime ideal(s)!embedded} prime ideals. In the example above, $\mathfrak{p}_2 = (x, y)$ is embedded.
...
\begin{proposition}\label{prop:localization-primary}
